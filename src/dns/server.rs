...
use crate::dns::wire::{builder, DnsQuery, DnsResponse, RecordType, ResponseCode};
...
/// Bind the DNS UDP socket. Call this synchronously on the main thread before
/// spawning the loop, so bind failures (notably EACCES on port 53 when running
/// non-root on Linux) are reported deterministically rather than fire-and-forget
/// inside a spawned task — the latter caused a UX bug where main printed
/// "DNS server listening on …" even when the server had failed to start.
pub async fn bind_dns_socket(listen_addr: SocketAddr) -> std::io::Result<Arc<UdpSocket>> {
    UdpSocket::bind(listen_addr).await.map(Arc::new)
}

/// Run the DNS server with caching and failover support. Thin wrapper that
/// binds then runs — for callers that want the legacy combined behavior. New
/// code should call `bind_dns_socket` + `run_dns_server_on` separately so the
/// bind error is observable on the main thread.
#[allow(dead_code)] // kept for external/test callers; the bin uses bind+run_on
...
pub async fn run_dns_server(
    ...
    let socket = bind_dns_socket(listen_addr).await?;
...
/// Run the DNS server loop on an already-bound socket.
///
/// `cache` is the shared `Arc<Mutex<DnsCache>>` instance — same Arc handed to
/// the proxy's `DnsClient` so cache entries warm both sides. The cache config
/// (`max_entries`, `min/max_ttl_seconds`, `enabled`) was already applied when
/// the cache was built; `cache_config` is passed in here only for the startup
/// banner.
#[allow(clippy::too_many_arguments)]
pub async fn run_dns_server_on(
    ...
    // Parse all upstream addresses. Empty list is valid: bunker becomes a
    // thin DNS server fronting the host's OS resolver
    // (`OsResolverHandler` mode in the design doc).
    ...
    let os_resolver_mode = upstream_addrs.is_empty();

    let rate_limiter = Arc::new(Mutex::new(DnsRateLimiter::new(security.clone())));
    let failover_config = Arc::new(failover_config);
    let security = Arc::new(security);
    let min_ttl_seconds = cache_config.min_ttl_seconds as u32;

    // Create the resolver with generics (zero-cost abstraction).
    // Only used in Upstream mode; kept around so the spawn-loop can reuse
    // the same Arc.
    let resolver = Arc::new(UdpDnsResolver::new((*security).clone()));

    if enable_logging {
        // Clean human-readable startup banner via println — keeps startup
        // output consistent with main.rs's other "X started/listening on Y"
        // lines (no timestamp / module-path noise from the tracing format).
        // Drops down to bunker's structured logger at debug! for anyone
        // who wants the same info in machine-readable form (RUST_LOG=debug).
        let upstream_summary = if os_resolver_mode {
            "OS-resolver (getaddrinfo)".to_string()
        } else {
            format!("{:?}", *upstream_addrs)
        };
        println!(
            "DNS server started: listen={} upstreams={} rate_limit={} \
             block_any={} block_zone_xfer={} cache_enabled={} cache_max_entries={}",
            listen_addr,
            upstream_summary,
            security.rate_limit_enabled,
            security.block_any_queries,
            security.block_zone_transfers,
        ...
        debug!(
            ...
            os_resolver_mode = os_resolver_mode,
    ...
    loop {
        ...
        if os_resolver_mode {
            // Mode 2: no upstreams — answer each query by calling getaddrinfo
            // and synthesizing a wire response. The server-level cache check +
            // serve-stale logic lives inside handle_dns_query_os so it works
            // exactly like the Upstream path from the operator's perspective.
            tokio::spawn(async move {
                send_or_servfail(
                    &socket_clone,
                    &buf,
            ...
            // Mode 1: forward to configured upstreams.
...
/// Common path for both Upstream and Os spawn arms: send the response on
/// success, or build + send a SERVFAIL on error. Extracted to keep the spawn
/// blocks tight.
async fn send_or_servfail(
    socket: &UdpSocket,
    query_buf: &[u8],
    src: SocketAddr,
    ...
    match result {
        ...
        Err(e) => {
            ...
            if let Some(response) = create_dns_error_response(query_buf, ResponseCode::ServFail) {
                let _ = socket.send_to(&response, src).await;
            }
        }
    }
...
pub async fn handle_dns_query<R: DnsResolver>(
    ...
    if enable_logging {
        // Per-query at DEBUG (was INFO) — keeps default `logging.level: info`
        // production output quiet. Set `logging.level: debug` (or
        // RUST_LOG=debug) to see every received query.
        debug!(
            id = id,
            client = %client,
            name = %query_name,
...
/// Handle a DNS query in OS-resolver mode: parse the query, call getaddrinfo
/// via `tokio::net::lookup_host`, build a wire response from the matching
/// records, cache it, and return the bytes. Mirrors the validation /
/// rate-limit-blocked / cache-hit / serve-stale flow of `handle_dns_query`
/// so an operator switching between Mode 1 and Mode 2 sees the same
/// behavioral shape (just a different source of records).
///
/// Non-A/AAAA queries get a `NOTIMP` response — bunker in this mode is a
/// thin getaddrinfo wrapper, not a full forwarder, so types the OS resolver
/// can't satisfy (MX, TXT, PTR, …) get an honest "not implemented" rather
/// than a fabricated empty answer.
pub async fn handle_dns_query_os(
    query_buf: &[u8],
    client: SocketAddr,
    security: &DnsSecurityConfig,
    cache: Arc<Mutex<DnsCache>>,
    ...
    let query = match DnsQuery::parse(query_buf) {
        ...
        Err(e) => {
            if enable_logging {
                warn!(client = %client, error = %e, bytes = query_buf.len(),
                      "DNS failed to parse query (os-resolver mode)");
            }
            return Err(format!("Failed to parse query: {}", e).into());
        }
    };
    ...
    if enable_logging {
        debug!(id = id, client = %client, name = %query_name, qtype = %query_type,
               "DNS query (os-resolver mode)");
    }

    // Validate (block ANY, AXFR/IXFR).
    if let Err(reason) = validate_dns_query(&query, security) {
        if enable_logging {
            warn!(id = id, client = %client, reason = %reason, "DNS query blocked");
        }
        return create_dns_error_response(query_buf, ResponseCode::Refused)
            .ok_or_else(|| "Failed to create error response".into());
    }

    // Only A and AAAA are answerable by getaddrinfo. Everything else gets
    // NOTIMP — honest rather than silently empty.
    if !matches!(query_type, RecordType::A | RecordType::AAAA) {
        if enable_logging {
            debug!(qtype = %query_type, client = %client,
                   "DNS qtype not supported in os-resolver mode → NOTIMP");
        }
        return create_dns_error_response(query_buf, ResponseCode::NotImp)
            .ok_or_else(|| "Failed to create NOTIMP response".into());
    }

    // Cache check — uses the same shared Arc<Mutex<DnsCache>> the proxy's
    // DnsClient::Os also writes to, so a proxy lookup warms the LAN-side
    // response and vice versa.
    {
        let cache_guard = cache.lock().await;
        if let Some(cached) = cache_guard.get(&query_name, query_type, id) {
            if enable_logging {
                debug!(name = %query_name, qtype = %query_type, client = %client,
                       "DNS cache hit (os-resolver mode)");
            }
            return Ok(cached);
        }
    }

    // getaddrinfo. Strip the trailing dot DnsQuery::parse adds — `lookup_host`
    // is happy without it and the OS expects bare hostnames.
    let bare_name = query_name.trim_end_matches('.');
    let resolve_result = tokio::net::lookup_host((bare_name, 0)).await;
    let ips: Vec<IpAddr> = match resolve_result {
        Ok(iter) => iter.map(|sa| sa.ip()).collect(),
        Err(e) => {
            // Resolution failure → NXDOMAIN. Serve-stale takes priority if
            // the operator has a stale entry from before the failure.
            if enable_logging {
                warn!(name = %query_name, client = %client, error = %e,
                      "DNS os-resolver lookup failed");
            }
            let cache_guard = cache.lock().await;
            if let Some(stale) = cache_guard.get_stale(&query_name, query_type, id) {
                if enable_logging {
                    warn!(name = %query_name, qtype = %query_type,
                          "DNS serving stale cache (os-resolver lookup failed)");
                }
                return Ok(stale);
            }
            drop(cache_guard);
            return create_dns_error_response(query_buf, ResponseCode::NxDomain)
                .ok_or_else(|| "Failed to create NXDOMAIN response".into());
        }
    };

    // Build a wire response with the matching family. `build_response_with_ips`
    // filters internally (A only keeps V4, AAAA only keeps V6), so a mixed
    // result from getaddrinfo Just Works.
    let response = builder::build_response_with_ips(query_buf, &ips, query_type, min_ttl_seconds);

    if enable_logging {
        let count = response.len();
        debug!(name = %query_name, qtype = %query_type, bytes = count,
               "DNS os-resolver response built");
    }

    // Cache the response. TTL=0 → DnsCache::put's clamp raises it to
    // min_ttl_seconds.
    {
        let mut cache_guard = cache.lock().await;
        cache_guard.put(&query_name, query_type, response.clone(), 0);
    }
...
mod tests {
    ...
    // ==================== handle_dns_query_os tests ====================

    #[tokio::test]
    async fn os_handler_resolves_localhost_a() {
        // `localhost` is always resolvable to 127.0.0.1 / ::1 on any reasonable
        // host, so this exercises the success path end-to-end.
        let query = build_dns_query("localhost", RecordType::A, 0x4001);
        let cache = Arc::new(Mutex::new(DnsCache::new(test_cache_config())));
        let security = test_security_config();
        let client: SocketAddr = "127.0.0.1:1234".parse().unwrap();

        let result = handle_dns_query_os(&query, client, &security, cache, 60, false).await;
        assert!(result.is_ok());
        let resp_bytes = result.unwrap();
        let resp = DnsResponse::parse(&resp_bytes).unwrap();
        assert_eq!(resp.response_code(), ResponseCode::NoError);
        assert!(
            resp.answer_count() >= 1,
            "localhost should have at least one A"
        );
        let ips = crate::dns::wire::extract_ips(&resp_bytes, RecordType::A);
        assert!(ips.contains(&IpAddr::V4(std::net::Ipv4Addr::LOCALHOST)));
    }

    #[tokio::test]
    async fn os_handler_nxdomain_for_invalid() {
        // RFC 2606 reserves .invalid for guaranteed non-resolution.
        let query = build_dns_query("nope.invalid", RecordType::A, 0x4002);
        let cache = Arc::new(Mutex::new(DnsCache::new(test_cache_config())));
        let security = test_security_config();
        let client: SocketAddr = "127.0.0.1:1234".parse().unwrap();

        let result = handle_dns_query_os(&query, client, &security, cache, 60, false).await;
        assert!(result.is_ok(), "should return NXDOMAIN response, not error");
        let resp_bytes = result.unwrap();
        let resp = DnsResponse::parse(&resp_bytes).unwrap();
    ...
    #[tokio::test]
    async fn os_handler_notimp_for_mx() {
        // MX queries aren't answerable via getaddrinfo → NOTIMP rather than
        // a synthesized empty answer.
        let query = build_dns_query("example.com", RecordType::MX, 0x4003);
        let cache = Arc::new(Mutex::new(DnsCache::new(test_cache_config())));
        let security = test_security_config();
        let client: SocketAddr = "127.0.0.1:1234".parse().unwrap();
        ...
        let resp = DnsResponse::parse(&bytes).unwrap();
        assert_eq!(resp.response_code(), ResponseCode::NotImp);
    }

    #[tokio::test]
    async fn os_handler_blocks_any_query() {
        let query = build_any_query("example.com", 0x4004);
        let cache = Arc::new(Mutex::new(DnsCache::new(test_cache_config())));
        let security = test_security_config(); // block_any_queries: true
        let client: SocketAddr = "127.0.0.1:1234".parse().unwrap();
    ...
    #[tokio::test]
    async fn os_handler_caches_response() {
        let query = build_dns_query("localhost", RecordType::A, 0x4005);
        let cache = Arc::new(Mutex::new(DnsCache::new(test_cache_config())));
        let security = test_security_config();
        let client: SocketAddr = "127.0.0.1:1234".parse().unwrap();

        // First call populates the cache.
        let first = handle_dns_query_os(&query, client, &security, Arc::clone(&cache), 60, false)
            .await
            .unwrap();

        // Cache entry should now exist for ("localhost.", A).
        let guard = cache.lock().await;
        let cached = guard.get("localhost.", RecordType::A, 0x4005);
        assert!(
            cached.is_some(),
            "first os_handler call should populate the cache"
        );
        drop(guard);

        // Second call returns the same bytes (modulo possible re-resolution
        // path; cache hit is the expected fast path).
        let second = handle_dns_query_os(&query, client, &security, cache, 60, false)
            .await
            .unwrap();
        // Both responses should parse to the same answer count.
        let r1 = DnsResponse::parse(&first).unwrap();
        let r2 = DnsResponse::parse(&second).unwrap();
        assert_eq!(r1.answer_count(), r2.answer_count());
    }

    #[tokio::test]
    async fn os_handler_malformed_query_errors() {
        let malformed = vec![0xFF; 4];
        let cache = Arc::new(Mutex::new(DnsCache::new(test_cache_config())));
        let security = test_security_config();
        let client: SocketAddr = "127.0.0.1:1234".parse().unwrap();

        let result = handle_dns_query_os(&malformed, client, &security, cache, 60, false).await;
        assert!(result.is_err());
    }
