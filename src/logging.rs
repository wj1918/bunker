...
pub fn init_logging(config: &LoggingConfig) -> LogGuard {
    ...
    // Precedence: RUST_LOG env var (ad-hoc debugging) > logging.level config >
    // built-in default "info". A malformed config value is logged-and-fallen-back
    // rather than panicked at startup so a typo in YAML doesn't take the proxy down.
    let env_filter = if let Ok(env) = std::env::var("RUST_LOG") {
        EnvFilter::try_new(&env).unwrap_or_else(|_| {
            eprintln!("WARNING: invalid RUST_LOG ({env:?}), falling back to 'info'");
            EnvFilter::new("info")
        })
    } else if let Some(cfg_level) = config.level.as_deref().filter(|s| !s.is_empty()) {
        EnvFilter::try_new(cfg_level).unwrap_or_else(|_| {
            eprintln!("WARNING: invalid logging.level ({cfg_level:?}), falling back to 'info'");
            EnvFilter::new("info")
        })
    } else {
        EnvFilter::new("info")
...
fn compress_rotated_logs(log_dir: &str, file_prefix: &str) {
    ...
    for entry in entries.flatten() {
        ...
        // `path.is_file()` would follow symlinks — letting an attacker who
        // can write to the log directory swap a benign rotated log for a
        // symlink to any file the bunker user can read, then have us
        // compress and rewrite it. Use DirEntry::file_type which calls
        // fstatat with AT_SYMLINK_NOFOLLOW (no separate stat). A narrow
        // TOCTOU window remains between this check and `compress_file`'s
        // File::open — closing it fully requires O_NOFOLLOW (Unix) /
        // FILE_FLAG_OPEN_REPARSE_POINT (Windows), which we defer behind
        // a direct libc dep when the hourly job's threat model warrants it.
...
fn cleanup_old_logs(log_dir: &str, file_prefix: &str, max_age_days: u64) {
    ...
    for entry in entries.flatten() {
        ...
        // Reject symlinks — see compress_rotated_logs for rationale. Without
        // this check, an attacker who can write to the log dir could swap a
        // benign rotated log for a symlink to an arbitrary file the bunker
        // user can delete, and we'd happily unlink it.
        let file_type = match entry.file_type() {
            Ok(ft) => ft,
            Err(_) => continue,
        };
        ...
        // Check file age via symlink_metadata so we never follow a symlink
        // installed between the file_type check above and this stat.
        let metadata = match fs::symlink_metadata(&path) {
            Ok(m) => m,
            Err(_) => continue,
        };
        if metadata.is_symlink() {
            // Defense in depth — file_type already rejected symlinks above,
            // but if a swap happened in the window in between, catch it here.
            continue;
        }
