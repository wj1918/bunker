...
//! - Sharded outer container (`ShardedSenderPool`) with `std::sync::Mutex` —
//!   the pool lock is never held across an `await`, so the tokio Mutex
//!   wakeup machinery was unnecessary overhead.
...
/// Sharded wrapper around `SenderPool` for reduced lock contention under high
/// concurrency. Each shard is independently guarded by a `std::sync::Mutex` —
/// safe because the inner `get`/`put`/`cleanup` operations never `.await`.
///
/// Shard count is fixed at compile time (16). The `max_total_connections`
/// budget is divided across shards so the global cap is preserved.
pub struct ShardedSenderPool {
    shards: Vec<Mutex<SenderPool>>,
}

impl ShardedSenderPool {
    const NUM_SHARDS: usize = 16;

    pub fn new(config: ConnectionPoolConfig) -> Self {
        let per_shard = ConnectionPoolConfig {
            max_total_connections: (config.max_total_connections / Self::NUM_SHARDS).max(1),
            ..config
        };
        let mut shards = Vec::with_capacity(Self::NUM_SHARDS);
        for _ in 0..Self::NUM_SHARDS {
            shards.push(Mutex::new(SenderPool::new(per_shard.clone())));
        }
        ShardedSenderPool { shards }
    }

    fn shard_idx(host: &str, port: u16, is_tls: bool) -> usize {
    ...
    pub fn get(
        ...
        let idx = Self::shard_idx(host, port, is_tls);
        self.shards[idx].lock().unwrap().get(host, port, is_tls)
    }
    ...
    pub fn put(
        ...
        self.shards[idx]
            .lock()
            .unwrap()
            .put(host, port, is_tls, sender, created_at);
    }

    pub fn cleanup(&self) {
        for shard in &self.shards {
            shard.lock().unwrap().cleanup();
        }
    }
