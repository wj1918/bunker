...
use crate::helpers::{apply_tcp_keepalive, error_response, error_response_close};
use crate::proxy::connect::handle_connect;
use crate::proxy::pool::ShardedSenderPool;
...
#[allow(clippy::too_many_arguments)] // hyper-path entry point bundles per-connection state; grouping into a struct just shifts the verbosity to the caller
pub async fn handle_client(
    ...
    sender_pool: Arc<ShardedSenderPool>,
    connect_timeout: Duration,
    preserve_header_case: bool,
    dns_client: Arc<DnsClient>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    // Disable Nagle's algorithm: a proxy writes small framed messages (response
    // headers, then body) and must not wait to coalesce them. Left on, Nagle +
    // the peer's delayed-ACK adds latency per request. nginx/HAProxy set this too.
    ...
    // Case preservation makes forwarded header names faithful to the client's
    // request (Title-Case for synthesized ones), at a per-request CPU cost.
    // Configurable via proxy.preserve_header_case (default true).
                ...
                async move {
                    ...
                    if max_requests > 0 {
                        ...
                        if current >= max_requests {
                            ...
                            return Ok(error_response_close(
                                429,
...
pub async fn proxy_request(
    ...
    if has_content_length && has_transfer_encoding {
        ...
        // RFC 7230 §3.3.3 requires closing on framing ambiguity — the stream
        // can no longer be trusted to delimit subsequent messages.
        return Ok(error_response_close(
            400,
    ...
    //
    // CHUNKED LIMITATION (hyper path only): chunked uploads bypass
    // `max_request_body_bytes` here because the SendRequest pool is pinned to
    // hyper's `Incoming` body and we can't substitute a `Limited<Incoming>`
    // without making the pool generic over body type. The engine path
    // (src/proxy/framing.rs::scan_message_limited) enforces the cap on both
    // Content-Length and chunked, so operators on the engine binary (the
    // default since build.cmd was updated) get full enforcement. If you
    // require the limit on chunked under hyper, run on the engine.
            ...
            if let Ok(length_str) = content_length.to_str() {
                if let Ok(length) = length_str.parse::<u64>() {
                    if length > security.max_request_body_bytes {
                        ...
                        // Close the connection: we haven't read the oversized
                        // body, so the next request on this socket would land
                        // in the middle of the unread payload.
                        return Ok(error_response_close(
                            413,
    ...
    if let Some(reason) = is_blocked_target(&host, Some(port), &security) {
    ...
    if logging.log_requests {
        // Per-request at DEBUG (was INFO) — see connect.rs for rationale.
        debug!(
            client = %client_addr,
            method = %method,
            host = %host,
    ...
    let resolved_addrs = match resolve_and_validate_host(&host, port, &security, &dns_client).await
    {
        Ok(addrs) => addrs,
    ...
    // Build the request to send to target. Borrow the path from the already-cloned
    // `uri` (independent of `req`, which build_target_request consumes) to avoid a
    ...
    // Try to get an existing sender from the pool. `ShardedSenderPool::get` is
    // a sync (std::sync::Mutex) call — safe because the inner critical section
    // never awaits.
    let (mut sender, created_at, from_pool) =
        if let Some((s, created)) = sender_pool.get(&host, port, use_tls) {
            if logging.log_requests {
                debug!(
                    host = %host,
                    port = port,
            ...
            // Create new connection and sender (connect_timeout cached at startup)
    ...
    if result.is_ok() && !sender.is_closed() {
        sender_pool.put(&host, port, use_tls, sender, created_at);
...
pub fn build_target_request(
    ...
    // Pass the host value straight into the header instead of building an
    // intermediate String: for default ports the borrowed `host` is used as-is,
    // and the non-default case formats directly into the HeaderValue.
    builder = if port == 80 || port == 443 {
        builder.header("Host", host)
    } else {
        builder.header("Host", format!("{}:{}", host, port))
    };
    builder = builder.header("X-Forwarded-For", client_addr.ip().to_string());

    // Copy headers from original request (except hop-by-hop headers and any
    // incoming X-Forwarded-For). hyper's Builder::header appends rather than
    // replaces, so an attacker-supplied XFF would otherwise land alongside the
    // proxy's value and let the origin trust a forged client IP — the
    // forward.rs/engine path filters XFF explicitly for the same reason.
    let hop_by_hop = [
        "connection",
        "keep-alive",
        "proxy-authenticate",
...
async fn create_sender(
    ...
    // Disable Nagle on the upstream leg too — same reasoning as the client side.
...
mod tests {
    ...
    fn test_hop_by_hop_headers_list() {
        // Mirror of the production list in build_target_request — keep these in sync.
        let hop_by_hop = [
            "connection",
            "keep-alive",
            "proxy-authenticate",
        ...
        // x-forwarded-for must be filtered so an attacker-supplied value doesn't
        // ride alongside the proxy's own XFF in the outgoing request (hyper's
        // Builder::header appends, it does not replace).
        assert!(hop_by_hop.contains(&"x-forwarded-for"));
