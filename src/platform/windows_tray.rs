...
fn show_context_menu(hwnd: isize) {
    unsafe {
        ...
        // Force a task switch back to our (hidden) tray window so the next
        // TrackPopupMenu invocation registers its selection. Per MSDN's
        // TrackPopupMenu docs and KB Q135788: a tray's owner-window only
        // truly becomes foreground after a benign message wakes the queue;
        // without this, the first click works but subsequent right-clicks
        // paint the menu yet TrackPopupMenu returns 0 (no selection), and
        // every menu item appears dead.
...
fn create_tray_icon(hwnd: isize) -> bool {
    unsafe {
        ...
        // MAKEINTRESOURCE encodes the resource ID into the pointer's low bits —
        // this isn't a dangling pointer, it's a Win32-defined sentinel value.
        // `std::ptr::dangling::<u16>()` would give align_of::<u16>(), not 1.
