...
pub struct ProxyConfig {
    ...
    /// Preserve the original case of forwarded header names (and emit Title-Case
    /// for synthesized ones). Faithful to the client's request — some
    /// case-sensitive upstreams need it — but costs per-request CPU. Set false
    /// for ~3% more throughput if your backends are well-behaved (case-insensitive
    /// per RFC 7230); headers are then forwarded lowercase.
    #[serde(default = "default_preserve_header_case")]
    pub preserve_header_case: bool,
    /// Use the experimental thread-per-core engine instead of the default hyper
    /// proxy path. Defaults to ON when the binary is built with the `engine`
    /// feature, and OFF otherwise — so a feature build runs the engine by default
    /// while the stock (no-feature) binary still boots on the hyper path. Set
    /// explicitly to override. The backend (io_uring on Linux, tokio elsewhere)
    /// is chosen at build time from the compiled features + target OS.
...
fn default_preserve_header_case() -> bool {
    true
}

/// Engine defaults ON only when compiled in (the `engine` feature), so the stock
/// binary stays on the hyper path and still boots with a default config.
fn default_engine() -> bool {
    cfg!(feature = "engine")
}
...
impl Default for ProxyConfig {
    fn default() -> Self {
        ProxyConfig {
            ...
            preserve_header_case: default_preserve_header_case(),
...
pub struct DnsConfig {
    /// Mirrors `proxy.enabled`: lets operators keep the DNS block in the
    /// file (so it's tunable in place) while flipping the subsystem off
    /// without commenting everything out. Defaults to true so configs
    /// that simply uncomment the `dns:` block keep working.
...
impl DnsConfig {
    ...
    ///
    /// Empty input returns an empty Vec — the caller (main.rs) treats that as
    /// the signal to run the DNS server in OS-resolver mode (Mode 2) instead
    /// of silently defaulting to a third-party DNS like Google's 8.8.8.8.
...
pub struct SecurityConfig {
    ...
    /// Maximum requests per connection (0 = unlimited, default 100).
    /// This caps how many keep-alive requests ride on a single TCP socket
    /// before bunker forces a reconnect — which is where the per-IP rate
    /// limiter is re-consulted. The default was lowered from 1000 to 100
    /// to narrow rate-limiter amplification (an allowed IP could previously
    /// run 1000× the configured `rate_limit.max_requests` between reconnects).
    /// Real keep-alive clients almost never exceed 100 reqs/conn; bots and
    /// load generators do, and those are exactly what tighter caps target.
...
fn default_max_requests_per_connection() -> u32 {
    100 // Reduced from 1000 to narrow rate-limit amplification; see field doc.
}
...
pub struct LoggingConfig {
    ...
    /// Tracing log-level filter. Accepts any EnvFilter expression — a bare
    /// level like "warn" or "error" applies globally, or a comma-separated
    /// per-module list like "warn,bunker::dns=debug,bunker::proxy::engine=info".
    /// Defaults to "info". The `RUST_LOG` environment variable, when set,
    /// takes precedence over this field for ad-hoc debugging.
    #[serde(default)]
...
pub fn default_dns_enabled() -> bool {
    true
}
...
#[allow(dead_code)] // kept for documentation / future templates; not used at runtime
                    // (no silent fallback — empty upstreams means Mode 2 OS-resolver)
...
"#;
        ...
        // Empty upstream config no longer silently inserts 8.8.8.8 — main.rs
        // treats an empty list as the signal to run the DNS server in
        // OS-resolver mode (Mode 2) and the proxy as DnsClient::Os.
        assert!(
            dns.get_upstreams().is_empty(),
            "no upstreams configured should yield empty Vec, not a default"
        );
    }

    #[test]
    fn dns_get_upstreams_returns_empty_when_unset() {
        // Both `upstream` and `upstreams` empty / unset → empty Vec, no
        // silent default. Caller decides what an empty list means.
        let dns = DnsConfig {
            enabled: true,
        ...
        assert!(dns.get_upstreams().is_empty());
    }

    #[test]
    fn dns_get_upstreams_merges_single_and_list() {
        let dns = DnsConfig {
            enabled: true,
            listen: default_dns_listen(),
            upstream: Some("8.8.8.8:53".to_string()),
            upstreams: vec!["1.1.1.1:53".to_string(), "9.9.9.9:53".to_string()],
            security: DnsSecurityConfig::default(),
            cache: DnsCacheConfig::default(),
            failover: DnsFailoverConfig::default(),
        };
        assert_eq!(
            dns.get_upstreams(),
            vec!["8.8.8.8:53", "1.1.1.1:53", "9.9.9.9:53"]
        );
    }
    ...
    fn test_security_config_default() {
        ...
        assert_eq!(config.max_requests_per_connection, 100); // 100 reqs/conn — see field doc on rate-limit amplification
    }
