    ...
    static int Main(string[] args)
    {
        ...
        try
        {
            ...
            else if (saveHr == 0 && policyHr == 1)
            {
                if (sigValid)
                {
                    Console.WriteLine("    >> WARN - CheckPolicy() flagged the file (SmartScreen reputation prompt)");
                    Console.WriteLine("    >> Signed binary; first-run users may see 'Don't run / More info'.");
                    Console.WriteLine("    >> NOT a hard block - Microsoft winget validation accepts this state for signed binaries.");
                    warned = true;
                }
                else
                {
                    Console.WriteLine("    >> FAIL - Unsigned binary flagged by CheckPolicy()");
                    Console.WriteLine("    >> Winget validation requires a valid signature; this WILL be blocked.");
                    exitCode = 1;
                }
            }
        ...
        if (exitCode == 0 && warned)
        {
            Console.WriteLine("File passes SmartScreen checks (with reputation warning - see [3] above).");
            Console.WriteLine("Signing identity is valid but lacks SmartScreen reputation; new users may see a");
            Console.WriteLine("'Don't run / More info' prompt until enough downloads accumulate.");
        }
        else if (exitCode == 0)
        {
